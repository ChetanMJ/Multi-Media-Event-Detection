...
if __name__ == '__main__':
    ...
    val_video = 'list/val'
    train_val_video = 'list/train_val'
    ...
    for file in files:
        ...
        if label == event:
            ...
            if label == 'P001' and P001_CNT<=25:
                P001_CNT = P001_CNT+1
                train_label_list.append('NOT_'+event)
                train_feature_list.append(all_video_features[vedio_index])
                train_videos_list.append(file.strip().split()[0])
                
            if label == 'P002' and P002_CNT<=25:
                P002_CNT = P002_CNT+1
                train_label_list.append('NOT_'+event)
                train_feature_list.append(all_video_features[vedio_index])
                train_videos_list.append(file.strip().split()[0])
                
            if label == 'P003' and P003_CNT<=25:
                P003_CNT = P003_CNT+1
                train_label_list.append('NOT_'+event)
                train_feature_list.append(all_video_features[vedio_index])
                train_videos_list.append(file.strip().split()[0])
    ...
    for kt in kernel_type:
        for rp in regparam:
            for gt in gamma_type:
            	average_prec = []
            	for proper_train_indices, val_indices in kf.split(train_feature_list):
            		proper_train_features = train_feature_list[proper_train_indices]
            		proper_train_labels = train_label_list[proper_train_indices]
            		val_features = train_feature_list[val_indices]
            		val_labels = train_label_list[val_indices]
        
            		SVM_classifier = svm.SVC(C=rp,  kernel=kt, gamma=gt, probability=True)
            		SVM_classifier.fit(proper_train_features, proper_train_labels)
            		predict = SVM_classifier.predict_proba(val_features)
            		prediction_score = predict[:,1]
            		val_lables_num = np.where(val_labels==event, 1, 0)
            		average_precision = sklearn.metrics.average_precision_score(val_lables_num, prediction_score)
            		average_prec.append(average_precision)
        
            	cross_val_AP = np.mean(average_prec)

            	if (best_ap == None) or (cross_val_AP > best_ap):
            		best_ap = cross_val_AP
            		best_kernel = kt
